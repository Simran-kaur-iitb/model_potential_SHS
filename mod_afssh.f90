...
subroutine init_cond
 ...
 !! write(6,*) "random", rnd
  ...
  enddo
!!  write(6,*) x, v, sig_p
 !! write(6,*) "mass, omega -x2", mass(2), omg(2)
  state=1
...
!!  k_coup(3)=7.5d23*wave_to_J*1.d20
...
subroutine pot_q(V,dv_dx,q,x)             !! V_HAB
  ...
  fac1=b_b*exp(-a_a*x(1))
  fac2=DA*(1.d0-exp((-n_A*(q-d_A)**2)/(2*q)))
  fac3=c_c*DA*(1.d0-exp((-n_B*(x(1)-q-d_B)**2)/(2*(x(1)-q))))
  V=fac1+fac2+fac3
  dv_dx(1)=-a_a*fac1+ (c_c*DA-fac3)*n_B*(x(1)-q-d_B)*(x(1)-q+d_B)/(2*(x(1)-q)**2)
  dv_dx(2)=0.d0 
...
subroutine draw_pes
  ...
  do i=1,50
    ...
    x(1)=2.55d-10+0.4d-10*i/50.d0
    do j=1,50
      !!x(2)=-0.5d-10+1.d-10*j/50.d0
      x(2)=-0.1d-10+0.9d-10*j/50.d0
     !!x(2)=100.d-12+40.d-12*j/50.d0
      call evaluate_variables(0)
      call compute_dij
      write(10,'(5es15.7)')x*1.d10,V_k(1:2)/wave_to_J
  ...
  do j=1,n
    x(1)=-0.5d-10+1.d-10*(j-1)/real(n-1)
     !! call evaluate_variables(0)
    write(20,'(3es15.7)')x(1)*1.d10,V_k/wave_to_J!,dabs(d_ij(1,2,1:2)/1.d10)
  enddo
...
write(6,*) vm/wave_to_J
 ...
 do j=1,nbasis
   ...
   write(200,'(5es15.7)')r_grid(j)*1.d10,(V1+V2+cl)/(kcal_to_J)
